...
public class MRFRepository(ILogger<MRFRepository> Logger, IMySQLConnectionString Settings) : IMRFRepository
{
    private readonly string _defaultConnectionString = Settings.MySQLConnectionString;

    /// <inheritdoc />
    public IMySQLConnection CreateMySQLConnection(string? SQLConnectionString)
    {
        return new MySQLConnectionWrapper(new MySqlConnection(SQLConnectionString ?? _defaultConnectionString));
    }
        ...
        IMySQLConnection SQLConnection, IDbTransaction? Transaction = null)
    {
        ...
        IEnumerable<TReturnType> _queryResult =
            await SQLConnection.QueryAsync<TReturnType>(Query.SQL, Query.DapperParameter.Parameters, Transaction);
        ...
        IMySQLConnection SQLConnection, IDbTransaction? Transaction = null)
    {
        ...
        TReturnType _queryResult =
            await SQLConnection.QueryFirstAsync<TReturnType>(Query.SQL, Query.DapperParameter.Parameters, Transaction);
        return _queryResult;
    }
    ...
    public async Task<TReturnType?> QueryFirstOrDefaultAsync<TReturnType>(IDapperQuery<TReturnType> Query,
        IMySQLConnection SQLConnection, IDbTransaction? Transaction = null)
    {
        Logger.LogDebug("[{Repository}] QueryFirstOrDefaultAsync: {Query}", nameof(MRFRepository),
            Query.GetType().Name);
        TReturnType? _queryResult =
            await SQLConnection.QueryFirstOrDefaultAsync<TReturnType>(Query.SQL, Query.DapperParameter.Parameters,
                Transaction);
        ...
        IMySQLConnection SQLConnection, IDbTransaction? Transaction = null)
    {
        ...
        TReturnType _queryResult = await SQLConnection.QuerySingleAsync<TReturnType>(Query.SQL, Query.DapperParameter.Parameters, Transaction);
        return _queryResult;
    }
        ...
        IMySQLConnection SQLConnection, IDbTransaction? Transaction = null)
    {
        ...
        TReturnType? _queryResult = await SQLConnection.QuerySingleOrDefaultAsync<TReturnType>(Query.SQL, Query.DapperParameter.Parameters,
                Transaction);
        return _queryResult ?? default;
    }
    ...
    public async Task ExecuteStoredProcedureAsync(IDapperStoredProcedure StoredProcedure,
        IMySQLConnection SQLConnection, IDbTransaction? Transaction = null)
    {
        Logger.LogDebug("[{Repository}] ExecuteStoredProcedureAsync: {Procedure}", nameof(MRFRepository),
            StoredProcedure.GetType().Name);
        ...
        IMySQLConnection SQLConnection, CancellationToken CancellationToken)
    {
        ...
        try
        {
            Logger.LogDebug("[{Repository}] Beginning transaction [{Transaction}]", nameof(MRFRepository),
                _transactionName);
            await DapperTransaction.SQLFunc(SQLConnection, this, _transaction);
            await _transaction.CommitAsync(CancellationToken);
            Logger.LogDebug("[{Repository}] Committed transaction [{Transaction}]", nameof(MRFRepository),
                _transactionName);
        }
        catch (Exception _ex)
        {
            await _transaction.RollbackAsync(CancellationToken);
            Logger.LogError(_ex, "[{Repository}] Transaction [{Transaction}] rolled back due to error",
                nameof(MRFRepository), _transactionName);
            throw;
        }
    }

    /// <inheritdoc />
    public async Task<TReturnType> ExecuteTransactionAsync<TReturnType>(
        IDapperTransaction<TReturnType> DapperTransaction,
        IMySQLConnection SQLConnection, CancellationToken CancellationToken)
    {
        string _transactionName = $"{DapperTransaction.GetType().Name}-{Guid.NewGuid()}";
        await using DbTransaction _transaction = await SQLConnection.BeginDbTransactionAsync();
        try
        {
            Logger.LogDebug("[{Repository}] Beginning transaction [{Transaction}]", nameof(MRFRepository),
                _transactionName);
            TReturnType _returnValue = await DapperTransaction.SQLFunc(SQLConnection, this, _transaction);
            await _transaction.CommitAsync(CancellationToken);
            Logger.LogDebug("[{Repository}] Committed transaction [{Transaction}]", nameof(MRFRepository),
                _transactionName);
            return _returnValue;
