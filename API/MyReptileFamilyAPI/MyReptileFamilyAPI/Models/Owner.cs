    ...
    string Password)
{
    ...
    public bool BasicIsValid(Range UsernameRange, Range PasswordRange, out RegisterUserResult ReasonOfResult)
    {
        // Check if password is valid and within a character limit
        if (string.IsNullOrWhiteSpace(Password) || Password.Length < PasswordRange.Start.Value ||
            Password.Length > PasswordRange.End.Value || !ComponentRegularExpressions.PasswordRegex().IsMatch(Password))
        {
        ...
        if (string.IsNullOrWhiteSpace(Username) || Username.Length < UsernameRange.Start.Value || Username.Length > UsernameRange.End.Value ||
            !ComponentRegularExpressions.UsernameRegex().IsMatch(Username) || string.IsNullOrWhiteSpace(Email) || !ComponentRegularExpressions.EmailRegex().IsMatch(Email))
        {
